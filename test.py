...
def main():
    layout = [
        [sg.Button("Update Status"), sg.Button("Exit")],
        [sg.Text("Status:"), sg.StatusBar(50, key="-STATUS-", relief=sg.RELIEF_SUNKEN)],
    ]

    window = sg.Window("Status Bar Example", layout)
...
def update_status_bar(window):
    ...
    for i in range(1, 11):
        time.sleep(0.1)
        new_text = f"Processing... {i * 10}% complete"
        window["-STATUS-"].update(new_text)
        window.refresh()
