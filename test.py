...
def detectMolecule(molecule,dipLocation,dipValue):
    ...
    for i,a in enumerate(molecWavelength):
        # print(i)
        if abs(min(dipLocation,key=lambda x:abs(x-a)) - a)<=0.0001:#Need to find what is qualified as "lines up", it doesn't have to be exactly the same, just very close
            absorptionFound.append(a)
            intensity.append(a)
    return (absorptionFound,intensity)

location,values=dipFinder(r"C:\Users\Tristan\Downloads\ExoSeer\ExoplanetDataTest.csv")
print("dips found")
lines,intensity=detectMolecule("H2O",location,values)  
    

 

plt.figure(figsize=(30, 10))
plt.plot(location,values)


plt.scatter(lines,[0]*len(lines),color="green")
plt.show()
